...
public class DerechoSalaQService extends Basica implements DerechoSalaQServiceRemote {
    ...
    public Future obtenerRegistros(String opunme, int opdata, Integer nivel, Integer unidpk, Integer ubicapk) { //opunme= Unidad Medica  OPdata=fecha en entero
        ...
        System.out.println("Hora Inicio Derecho de Sala Quirofano:" + imprimirHora().toString());
        ...
        try {
            ...
            try {
                ...
                Logger.getLogger(DerechoSalaQService.class.getName()).log(Level.SEVERE, null, ex1);
            }
            Logger.getLogger(DerechoSalaQService.class.getName()).log(Level.SEVERE, null, ex);
        }
        bag.clear();
        listaDetalles.clear();

        System.out.println("Hora Fin Derecho de Sala Quirofano:" + imprimirHora().toString());
    ...
    private void aplicarRegla(ParteOperatorio parOp) {
        ...
        //se debe buscar el mayor valor 
        int coeficiente = minutosBuscar / 420;
        int residuo = minutosBuscar % 420;

        double valorCobrarCoeficiente;
        double valorCobrarResiduo;
        ...
        if (residuo != 0) {
            ...
            valorCobrarResiduo = tarifario.getVALORTARIFARIO();

            CodigosTiempo codTiempo = new CodigosTiempo();
            codTiempo.setCodigoTarifario(codTarifarioResiduo);
            codTiempo.setValorUnitario(valorCobrarResiduo);
            codTiempo.setCantidad(1);
            codTiempo.setValorCobrar(valorCobrarResiduo);
        ...
        if (coeficiente != 0) {
            ...
            valorCobrarCoeficiente = tarifario.getVALORTARIFARIO();

            CodigosTiempo codTiempo = new CodigosTiempo();
            codTiempo.setCodigoTarifario(codTarifarioCoeficiente);
            codTiempo.setValorUnitario(valorCobrarCoeficiente);
            codTiempo.setCantidad(coeficiente);
            codTiempo.setDescripcion(tarifario.getDESCRIPCION());
            double valorCobrar = coeficiente * valorCobrarCoeficiente;
    ...
    @Override
    public List<Procedimientos> obtenerProcedimientos(int unidMed, int fechaOp) {
        ...
        params.put("unidMed", unidMed);
        params.put("fechaOp", fechaOp);
        List<Procedimientos> procedimientosList = queryExecutor.executeQuery(sqlString1, params, Procedimientos.class);
